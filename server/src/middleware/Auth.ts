...
export const IsUser = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!user || user?.role !== "User" || !user.restaurant.isActive)
            return res.status(401).json({ message: "Unautorized Access!" });
...
export const IsModifier = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
        if (req.user?.role == "User") {
            ...
            if (!user || !user.canModify || !user.restaurant.isActive) {
...
export const IsRestaurantOrUser = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
        if (!req.user?.email) return res.status(401).json({ message: "Unauthorized Access!" });
        if (req.user?.role == "User") {
            const user = await prisma.user.findUnique({
